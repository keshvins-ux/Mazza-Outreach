      ...
      <div style={{background:"linear-gradient(135deg,#1E3A5F 0%,#0F2442 100%)",padding:isMobile?"10px 12px":"14px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 4px 24px rgba(15,36,66,0.18)",flexWrap:"nowrap",gap:8,overflowX:"hidden"}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"nowrap",overflowX:"auto",WebkitOverflowScrolling:"touch",msOverflowStyle:"none",scrollbarWidth:"none",paddingBottom:2}}>
          ...
          {(portal==="sales"||(!portal&&currentUser?.sales)) && currentUser?.sales && (<>
            ...
            <button onClick={()=>{setPortal("sales");setTab("analytics");}} style={tabBtn(tab==="analytics"||tab==="sales_growth"||tab==="sales_exec")}>📈 Analytics</button>
          ...
          <div style={{position:"relative",flexShrink:0}} ref={dropRef}>
            ...
            {showDrop && (
              <div style={{position:"absolute",right:0,top:"calc(100% + 8px)",background:"#fff",borderRadius:10,border:"1px solid #E2E8F0",boxShadow:"0 8px 24px rgba(0,0,0,0.12)",minWidth:140,zIndex:999,overflow:"hidden"}}>
                ...
                <button onClick={()=>{ try{localStorage.removeItem(SESSION_KEY);}catch{} setCurrentUser(null); setShowDrop(false); }}
      ...
      {(tab==="analytics"||tab==="sales_growth"||tab==="sales_exec") && (
        <div>
          <div style={{display:"flex",gap:0,borderBottom:"1px solid #E2E8F0",background:"#fff",padding:"0 28px"}}>
            {[{id:"growth",label:"📊 Sales Intelligence"},{id:"exec",label:"📋 Weekly Summary"}].map(t=>(
                      ...
                      return (
                        <div key={s.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",background:"#FFF7F7",borderRadius:10,border:"1px solid #FEE2E2"}}>
                          ...
                          <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                            ...
                            <div style={{fontSize:10,color:"#94A3B8"}}>{s.delivery.replace("DELIVERY DATE: ","")}
                          </div>
                        </div>
                      );
                    })}
                    ...
                    {filteredSO.slice(0,100).map((s,i)=>{
                      ...
                      return (
                        <tr key={s.id} style={{borderTop:"1px solid #F1F5F9",background:isLate?"#FFF7F7":i%2===0?"#fff":"#FAFAFA"}}>
                          ...
                          <td style={{padding:"10px 16px",color:isLate?"#EF4444":"#64748B",fontWeight:isLate?700:400,whiteSpace:"nowrap",fontSize:11}}>{isLate?"⚠️ ":""}s.delivery!=="-"?s.delivery.replace("DELIVERY DATE: ",""):"-"
