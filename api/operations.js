...
function getActiveFG(snap, intake, liveStatus) {
  const fgNeeded = {};
...
export default async function handler(req, res) {
  ...
  try {
    const [snapRaw,intakeRaw,soLiveRaw,bomRaw,stockRaw,ivRaw] = await Promise.all([
    ...
    const fgNeeded = getActiveFG(snap, intake, liveStatus);
    ...
    if (type === 'production') {
      ...
      for (const [code, p] of Object.entries(snap)) {
        ...
        const doneOrders = (p.orders||[]).filter(o=>isDone(liveStatus[o.soNo]));
