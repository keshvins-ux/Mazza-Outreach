...
async function resolveCustomerCode(redis, customerCode, soDocno) {
  ...
  const raw = await redis.get('mazza_so');
  const list = raw ? JSON.parse(raw) : [];
  const so = list.find(s => s.id === soDocno || s.docNo === soDocno);
  return so?.customerCode || so?.code || null;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'invoice') {
      ...
      if (!customerCode) return res.status(400).json({ error: 'Cannot determine customer code.' });
    ...
    if (type === 'do') {
      ...
      const payload = {
        ...
        // docref1: SO number + delivery progress — key field for SQL filtering
        docref1: deliveryProgress,
        docref2: `SO: ${soDocno || ''} | DEL: ${delDate.split('-').reverse().join('/')}${doLabel}`,
        note: note || '',
