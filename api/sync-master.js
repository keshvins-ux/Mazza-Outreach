...
// Stop when empty page returned — NOT when page < limit
async function fetchAllPages(endpoint) {
  ...
  while (true) {
    ...
    if (!items.length) break; // empty page = all done
...
export default async function handler(req, res) {
  ...
  try {
    // -- CUSTOMERS ------------------------------------------------------------
    try {
      const rawC = await fetchAllPages('/customer');
      const customers = rawC
        .filter(c => c.code && c.code !== '----')
        .map(c => ({ code: c.code, name: c.companyname || c.name || c.code, phone: c.phone1 || '', active: !c.cancelled }))
    ...
    // -- STOCK ITEMS -----------------------------------------------------------
    try {
      const rawS = await fetchAllPages('/stockitem');
      const stockItems = rawS
    ...
    } catch(e) { results.errors.push(`StockItems: ${e.message}`); }

    const now = new Date().toISOString();
    await redis.set('mazza_master_updated', now);

    return res.status(200).json({ success: true, ...results, updatedAt: now });
